...
# File locking so two students saving at the exact same instant can't clobber
# each other's entry (a plain read-modify-write on a shared file has a race:
# both reads happen before either write, so the second write can silently
# discard the first student's update). fcntl is POSIX-only (Linux/Mac,
# including Streamlit Community Cloud) — on Windows this quietly no-ops and
# behaves exactly as before, so local dev on any OS still works.
try:
    import fcntl
...
def save_team_members(username, value):
    ...
    Holds an exclusive lock across the whole read-modify-write so a
    concurrent save from another student can't overwrite this one (or vice
    versa)."""
    try:
        # "a+" creates the file if missing WITHOUT truncating it — unlike a
        # separate "does it exist? if not, create with 'w'" check, there's no
        # gap where two students racing past that check could both open in
        # truncating mode and wipe each other's already-written data.
        with open(TEAM_MEMBERS_FILE, "a+") as f:
            ...
            try:
                ...
                f.truncate()
                json.dump(data, f)
                # Flush BEFORE unlocking — otherwise the write can still be
                # sitting in Python's buffer when the lock releases, letting
                # a second thread/process start its own read before this
                # write has actually reached the file (the exact race the
        ...
        with st.expander(f"{c['name']} ({c['ticker']}) — {c['y2_label']}", expanded=(len(companies) <= 2)):
            ...
            if progress_store_available():
                if saved_at:
                    ...
                    st.caption(
                        " Autosave is set up but the last save attempt failed — your answers "
